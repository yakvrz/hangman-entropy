...
optimize_subject_drop <- function(data_by_drop, drops, id, measure){
  ...
  if(measure == "firstfix"){
    model_by_drop <-
      map(subj_data_by_drop,
          ~lm(log_firstfix_dur ~ firstfix_entropy_z + log_freq + length + firstfix_center_diff_fct,
              data = .x))
    
  } else if(measure == "refix"){
    model_by_drop <-
      map(subj_data_by_drop,
          ~glm(refix ~ firstfix_entropy_z + log_freq + length + firstfix_center_diff_fct,
               data = .x, family = "binomial", nAGQ = 0))
  }
  
  summary_by_drop <- map(model_by_drop, ~tidy(.x, conf.int = TRUE))
  # statistic_by_drop <- map_dbl(summary_by_drop, ~.x$statistic[2])
  # optimal_drop <- drops[which.max(abs(statistic_by_drop))]
  return(summary_by_drop)
}
